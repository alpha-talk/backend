...
- **core-api 7모듈 구현 완료 (M1~M5)**: auth·search·subscription·stream·notification·community·stockinfo — REST 23개 엔드포인트 + 쓰기 레이트리밋·멱등키(명세 §1.5·§1.6). 알림은 fan-out-on-read(ADR A4), 글은 더블라이트(ADR A6), 주/월봉은 일봉 파생(ADR A8). 테스트 211개(E2E 포함) 통과. 남은 것: M6 경화(CORS 화이트리스트·메트릭·배포)와 springdoc OpenAPI 상호 검증(§0).
