...
- **뉴스 파이프라인 N0~N6 구현 완료**: `:worker-ingest`(RSS·네이버 검색 수집→정규화→XADD + 다이제스트 트리거 — 텍스트 사전 매핑은 v0.6에서 제거, 종목 후보는 소스 부여만. 테스트 38개) + `:worker-llm`(소비→클러스터링(pgvector)→LLM 요약·감성→scope 사다리 fan-out→persist·publish·ack + 일일 브리핑·DLQ, 테스트 91개). 운영 LLM·임베딩은 미구성 시 fail-closed한다. 로컬 LLM은 Claude CLI 구독이 기본이고 Codex CLI 구독을 선택할 수 있으며, fake LLM은 `provider=fake` 명시 opt-in이다(테스트는 fake 사용). 상세·잔여 설정은 [md/alphatalk_news_worker_spec.md](md/alphatalk_news_worker_spec.md) §9.
