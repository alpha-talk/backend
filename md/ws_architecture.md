...
판단이 갈릴 때 돌아오는 여섯 가지 기준이다. 아래 모든 섹션의 결정은 이 원칙에서 따라 나온다.
...
6. **경계는 포트로, 도메인은 인프라를 모른다** — Redis·브로커·JWT 라이브러리 같은 인프라는 전부 우리가 정의한 인터페이스(포트) 뒤에 둔다. 도메인 로직(수요 전이, relay 라우팅)은 Lettuce도 `SimpMessagingTemplate`도 모르는 채로 테스트한다. (SOLID 적용 — §3)
...
> 불변 규칙 6("인메모리 상태는 DemandRegistry 단일 소유")과 충돌하지 않는다. 상태의 **소유**는 여전히 `DemandRegistry` 하나다. 떼어낸 것은 상태가 아니라 *구독을 실행하는 부수효과*와 *채널별 발행 로직*이다.
...
- **인증 방식 교체**: HS256 → RS256/Nimbus 전환 시 `TokenVerifier` 구현만 갈아 끼운다(§7 합의 안건 대응).
- **관심목록 소스 교체**: 임시 Redis → core-api REST/DB 전환 시 `WatchlistResolver` 구현만 갈아 끼운다.

### 3.3 LSP — 구현은 계약을 배신하지 않음

포트마다 구현이 바뀌어도 성립해야 하는 계약을 문서화한다(§4.5). 예를 들어 `WatchlistResolver.resolve`는 **미존재 유저에 빈 Set 반환(널·예외 아님)**, `TokenVerifier.verify`는 **유효 시 userId, 무효 시 정해진 예외**다. 임시 구현과 실제 구현이 이 계약을 공유하므로 서로 대체할 수 있다.
...
- `SessionEventListener` — Spring 이벤트를 포트 호출로 번역하고 **세션 수명 정책**(CONNECTED에 출석 등록, 첫 구독 시 해소·부착, 해소 실패 시 빈 목록)을 소유한다. 이 정책을 `DemandRegistry`에 넣지 않는 이유는 둘이다. resolve는 I/O(≤1s)라 레지스트리의 락 규율(락 안에서는 빠른 전이와 `ChannelSubscriber` 호출만)과 양립하지 않는다. 또 "어디서 가져오나"는 인덱스 불변식이 아니라 수명 정책이다(§11.4의 상태+불변식 응집 원칙). 구현이 하나뿐이고 교체점이 없어 인터페이스를 두지 않는다.
...
계층별로 무엇이 어떤 책임을 지는지. **포트**로 표시된 타입이 교체점이자 테스트 seam이고(§3.5), 나머지는 그 뒤에 숨는 구현이다.
...
| `RedisDemandSynchronizer` | 구현 | 단일 스레드가 주기(5s)·트리거마다 스냅샷을 Redis 해시에 **전체 재기록**(Lua DEL+HSET 원자, TTL 연장) 후 직전 기록과 비교해 0↔1 전이가 있었던 sync만 `demand:updated` 1건 발행(payload `{gwId, ts}`, v0.21) — 유실·드리프트는 다음 주기에 자가 치유. gwId(부팅마다 발급)·`gw:alive` 갱신·`gw:registry` 자기 등록(멱등 SADD)·stop 시 키 DEL + SREM 소유. 주기·TTL은 서비스 설정이 아니라 `:contracts`의 `DemandTiming`이 소유한다(worker-price의 복구 시간 검증식과 어긋나지 않도록) |
...
게이트웨이의 인메모리 상태는 아래가 전부다. 전부 살아있는 세션에서 파생되므로 영속화하지 않고, 재기동하면 0에서 다시 쌓인다(§1.5).
...
각 이벤트가 인덱스를 어떻게 바꾸고, 그 결과 어떤 Redis 구독을 켜고 끄는지. 부수효과는 전부 포트 호출이라 Redis 없이 검증할 수 있다(§3.5).
...
정합성 버그(refcount 꼬임 → 유령 구독·구독 유실)가 성능보다 훨씬 비싸다. 그래서 쓰기는 통째로 직렬화하고, 틱마다 도는 읽기 하나만 락 밖으로 뺀다.
...
- 락 최적화는 S7 스모크에서 병목으로 확인될 때만 한다. CHM 대신 락을 고른 자세한 근거는 §11.3.

### 5.4 스레딩 모델

블로킹 I/O를 어느 풀에서 지불하느냐가 이 모듈의 지연을 좌우한다(§11.6). 그래서 풀별로 무엇이 도는지 못 박아 둔다 — 아래 배치에서 벗어나는 작업을 추가하지 않는다.
...
§4의 컴포넌트가 실제로 어떤 순서로 맞물리는지. 대괄호는 그 구간이 도는 스레드다.

### 6.1 접속 — 출석은 CONNECTED에, 수요는 첫 SUBSCRIBE에

두 기록을 각 사실이 확정되는 시점에 나눠 적는다. **출석부**(세션↔유저 — 프레즌스·메트릭·broadcast-filter가 소비)는 세션이 성립되는 CONNECTED에, **수요**(관심목록 → Redis 구독)는 클라가 실제로 받기 시작하는 첫 SUBSCRIBE에 적는다.
...
- **attachWatchlist는 출석부에 살아있는 세션만 받는다** — resolve 도중 disconnect가 지나가면 no-op이라 유령 세션이 생기지 않는다.
- 부착은 유저 단위 1회(needsWatchlist가 게이트). 동시 SUBSCRIBE 레이스는 중복 resolve 1회로 무해하다. 미부착 유저의 `watchlist:updated` diff는 pendingDiffs에 압축 버퍼링했다가 부착 시 병합한다(§11.6).
...
처리 방식은 둘 중 하나다 — 세션을 끊거나, 한 건을 버리거나. 게이트웨이는 재전송·버퍼링으로 신뢰성을 흉내 내지 않으므로(§1.4), 어느 쪽이든 유실 복구는 클라의 REST가 맡는다. 프로토콜 위반은 즉시 끊어 잘못된 클라를 빨리 드러내고, 데이터 한 건의 실패는 드랍한 뒤 relay를 계속한다.
...
환경마다 달라지는 값은 전부 `WsProperties`로 외부화한다(§4.1). 시크릿은 환경변수로만 주입하고 커밋하지 않는다(NFR-07, §10).
...
> **포트를 별도 `port/` 패키지에 몰지 않는다.** 인터페이스는 그것이 표현하는 도메인 옆에 둔다(응집). 예를 들어 `ClientMessageSink`는 `client/`에 둔다. 단, **여러 서버가 공유하는 포트는 공유 라이브러리가 소유**한다 — `TokenVerifier`/`TokenIssuer`는 core-api와 공유하는 계약이므로 `:auth-jwt`가 갖는다.
...
```
> 코어와 자동구성을 한 모듈에 두는 이유: 소비자가 전부 Boot 앱이라 모듈 분리(core/starter 2단)는 과설계다. 코어 클래스는 여전히 Spring 없이 테스트된다.

---

## 10. 이 설계가 지키는 상위 요구사항

앞선 결정들이 어떤 요구를 지키려는 것인지 되짚는 표다. 설계를 바꿀 때는 오른쪽 대응이 깨지지 않는지 먼저 본다.
...
§11.1의 귀결이다. `/user/queue/quote` UNSUBSCRIBE는 **브로커 계층에서만 유효**(그 세션으로 전달 중단)하고, Redis 수요는 유지된다. 관심목록 수요를 끄는 정당한 경로는 연결 종료, 그리고 REST로 관심목록을 지우는 것(→ `watchlist:updated`) 둘뿐이다. 방 토픽은 UNSUBSCRIBE가 두 계층 모두 해제한다. 이 비대칭은 `DemandRegistryTest`("방 구독이 아닌 subId의 UNSUBSCRIBE - 무해")로 고정되어 있다.
...
- CHM은 연산 하나의 원자성만 보장한다. 이 클래스의 변경은 맵 4개 + `ChannelSubscriber` 부수효과에 걸친 **트랜잭션**이다. CHM만 쓰면 두 가지가 깨진다. 하나는 check-then-act 레이스로 접속 없는 유저의 채널이 영원히 구독되는 **유령 구독**, 다른 하나는 전이 판정 순서와 Redis 명령 실행 순서가 어긋나는 **구독 유실**(1→0의 unsubscribe가 0→1의 subscribe보다 늦게 실행)이다. 락은 판정과 부수효과 실행을 한 임계구역에 묶어 이를 차단한다.
...
- 쓰기 빈도(접속/구독 변경, 초당 수십 건)에서 단일 락 경합은 사실상 0이다. 락 세분화는 S7에서 병목으로 확인될 때만 한다.
...
- 역할 분리는 이미 **인터페이스 레벨**(ISP — `DemandQuery`/`DemandMutator`)에서 하고 있다. "인터페이스는 좁게, 구현은 불변식 단위로 응집"은 `JwtTokenProvider`(TokenIssuer+TokenVerifier)와 같은 패턴이다.
...
- 성격은 다르다 — quote/stream/post는 **데이터 평면**(클라에게 전달할 콘텐츠, 드랍 무해), `watchlist:updated`는 **제어 평면**(게이트웨이 자신의 수요 인덱스를 바꾸는 명령)이다.
- 그럼에도 현재는 단일 `MessageRouter` + 핸들러 맵으로 균일하게 처리한다(OCP 균일성, 코드 최소). 제어 리스너를 컨테이너에 직접 등록하는 분리안은 §11.4의 DI 순환도 근본 제거하지만, 채널 하나를 위해 등록 경로가 이원화되는 비용이 있어 보류한다.
...
- **블로킹은 제거가 아니라 이전이다**: 블로킹 스택(MVC + 동기 Lettuce)에서 Redis I/O는 반드시 어떤 스레드를 점유한다 — 고를 수 있는 건 지불하는 풀뿐이다. 전송 워커는 프레임 단위로 빌리는 공유 풀(기본 200)로 outbound(코어×2)의 10배라, 폭풍의 Redis I/O가 워커당 수 ms로 분산되고 틱 전달과 격리된다. Tomcat 워커의 블로킹 I/O는 서블릿 스택의 설계 중심(모든 REST 핸들러가 하는 일)이며, 제로 블로킹이 필요해지면 그건 WebFlux 전환의 문제다(v0.2에서 기각).
- **Redis 장애 방어**: 진짜 위험은 폭풍이 아니라 Redis 자체가 느려질 때다. 포트 계약(블로킹 ≤ 1s, §4.5)을 `spring.data.redis.timeout: 1s`로 강제한다 — Lettuce 기본(60s)대로면 장애 시 워커가 장시간 물려 풀 고갈로 번진다.
- **잔여 사항**: outbound에 남은 Redis I/O는 프레즌스 쓰기(SADD/EXPIRE, 1s 상한)뿐. S7 재접속 폭풍 시나리오에서 최종 검증한다.
...
- **우리가 틀렸던 것**: "`setPreserveReceiveOrder` API는 존재하지 않는다"는 반박(1·2차 검증이 특정 클래스만 뒤지거나 grep 출력을 잘라 놓친 오류). 실제로는 **`StompEndpointRegistry.setPreserveReceiveOrder`(공개 설정 API)** 가 있으며, 내부적으로 `StompSubProtocolHandler`에 플래그를 전달하고 프레임워크(`WebSocketMessageBrokerConfigurationSupport`)가 `OrderedMessageChannelDecorator.configureInterceptor`까지 **자동 결선**한다(바이트코드 확인).
