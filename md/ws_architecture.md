...
| `RedisDemandSynchronizer` | 구현 | 단일 스레드가 주기(5s)·트리거마다 스냅샷을 Redis 해시에 **전체 재기록**(Lua DEL+HSET 원자, TTL 연장) 후 직전 기록과 비교해 0↔1 전이만 `demand:updated` 발행 — 유실·드리프트는 다음 주기에 자가 치유. gwId(부팅마다 발급)·`gw:alive` 갱신·`gw:registry` 자기 등록(멱등 SADD)·stop 시 키 DEL + SREM 소유. 주기·TTL은 서비스 설정이 아니라 `:contracts`의 `DemandTiming`이 소유한다(worker-price의 복구 시간 검증식과 어긋나지 않도록) |
