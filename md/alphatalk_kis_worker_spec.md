...
- **채널 검사와 침묵 상태 변경은 원자적이어야 한다**: 채널 선택은 maintain 스레드가, 틱 수신은 WS 프레임 스레드가 만진다. "현재 채널의 틱인가"를 확인한 뒤 침묵 상태를 바꾸는 사이에 전환이 끼어들면, **이전 채널의 틱이 전환 후에 반영되어** 침묵 감시가 영구히 꺼진다 — 그 종목은 새 채널이 조용해도 다시 강등되지 않아 WS도 REST도 받지 못하는 사각지대가 된다. 첫 틱 처리(채널 확인 + 침묵 해제)는 구독 조정과 같은 락 안에서 한 번에 한다. 이미 틱이 흐르는 종목은 락 없이 즉시 빠져나가므로 뜨거운 경로에는 부담이 없다(락은 종목당 첫 틱까지만).
...
*KIS 수집 워커 명세 v0.6 — Redis 계약 v0.16·WS API v0.6·core-api 명세 v0.3과 정합. KIS 수치는 2026-07 공식 샘플 대조 기준이며 §9 항목은 실계정 재확인 대상.*
