...
- **침묵 감지로 배운다**: KIS가 NXT 상장 명단을 주지 않으므로(§2.6) 기록이 없는 종목은 통합으로 등록한 뒤 **`silence-ms`(기본 20s) 동안 틱이 하나도 없으면 KRX 전용으로 재등록**한다. 그 뒤 KRX에서 틱이 오면 그때 `J`로 확정 기록한다 — **침묵 자체는 약한 증거이므로 기록하지 않는다**(거래가 뜸한 종목도 조용하다). 통합에서 틱이 오면 `UN`으로 확정한다.
