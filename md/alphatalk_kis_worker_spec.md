...
- **수집 창은 08:00–20:00**이다 — §2.7의 수신 세션 전체(NXT 프리 08:00–08:50 · KRX/NXT 메인 09:00–15:30 · NXT 애프터 15:30–20:00 · KRX 시간외 15:40–18:00)를 **연속으로** 담는다. 실시간 틱은 이 창 전체를 푸시하는데 봉이 정규장만 있으면 장외 시간에 차트가 비고 시세 핀만 움직이는 불일치가 생긴다. 세션 사이 공백(08:50–09:00 등)은 봉이 없는 구간으로 자연스럽게 비고, 전방 페이징이 빈 창을 건너뛰며 전진한다(희소 데이터와 같은 처리). 하루 최대 봉 수는 **720**이다.

- **소스 API**: 당일은 `GET /uapi/domestic-stock/v1/quotations/inquire-time-itemchartprice` · TR `FHKST03010200` · `FID_INPUT_HOUR_1`(기준 시각) → **1콜 최근 30개 1분봉**. **시장 구분 `FID_COND_MRKT_DIV_CODE`는 실전에서 `UN`(통합)**이다 — 기본값 `J`는 KRX 전용이라 NXT 프리·애프터 봉이 아예 오지 않는다(창만 08:00–20:00으로 넓히면 빈 구간을 완주로 오판한다). 모의(vts)는 통합 지원이 미확인이라 `J`로 두고 정규장만 쌓는다 — 실시간 TR을 env별로 나누는 §2.3과 같은 원칙이며, vts의 `UN` 지원은 §9.10에서 확인한다. 과거 일자는 주식일별분봉조회 `GET /uapi/domestic-stock/v1/quotations/inquire-time-dailychartprice` · TR `FHKST03010230`(**실전 전용**). 원천은 KIS 공식 분봉이다 — 자체 틱 집계로 분봉을 만들지 않는다. §0의 "틱 비영속" 원칙은 유지된다: 틱을 대체하는 영속형이 봉이고, 그 봉의 값은 KIS가 준다.
...
10. **분봉 시장 구분 코드(§2.6)** — `FID_COND_MRKT_DIV_CODE`의 모의(vts) `UN` 지원 여부, 그리고 실전 `UN` 응답이 NXT 프리(08:00–08:50)·애프터(15:30–20:00) 봉을 실제로 포함하는지를 실응답으로 확정한다. 일봉(`FHKST03010100`)·현재가(`FHKST01010100`)는 아직 `J`(KRX)를 쓰므로, 분봉만 통합으로 가면 같은 날 일봉 OHLCV와 분봉 합계가 어긋난다 — 통합 전환 범위를 여기서 함께 정한다.
