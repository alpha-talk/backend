...
- **부분 성공은 성공이 아니다 — 날짜 키 스냅샷 잡의 공통 규칙**:
  ...
  - `valuation_daily`·`investor_flow_daily`는 실행 전에 **오늘 `stock_master_sync` SUCCESS 행이 있는지** 확인하고, 없으면 부분·낡은 유니버스를 처리하지 않고 FAILED로 남긴다. "행이 있는데 실패했으면"이 아니라 **"SUCCESS가 있어야"**로 잡는 이유는, 스테일 락으로 마스터 호출이 `runs.start` 전에 스킵되면 행 자체가 안 생기기 때문이다 — 행 부재를 정상으로 보면 어제 유니버스로 그날을 SUCCESS로 굳혀 그날 신규 상장 종목이 누락된다. `fail_count`는 보지 않는다: 이 규칙 이전 버전이 남긴 `SUCCESS + fail_count>0` 행을 막으면 `start`가 SUCCESS 행을 재시작하지 않아 배포 당일이 교착된다.
