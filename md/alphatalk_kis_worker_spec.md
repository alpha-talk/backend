...
- **일내 반복 잡 예외**: `invest_opinion_sync`(10분 주기)는 `(job, run_date)` SUCCESS 스킵을 적용하지 않는다 — 실행 이력만 기록하고 매 회 실행한다. 멱등은 잡 내부의 upsert·미발행 스캔(§3.3)이 담당한다. ShedLock은 동일하게 적용해 실행 중인 회차가 있으면 다음 트리거를 시작하지 않는다. lock TTL은 **트리거 간격보다 길게** 둔다 — 회차가 늘어져도 락이 살아 있어야 다음 트리거가 동시 실행 대신 `opinion_sync_overrun`으로 떨어진다. 데드라인은 루프 사이에서만 검사하므로 단일 연산이 늘어지는 것까지 막지는 못하고, 그래서 TTL이 최종 방어선이다. 잡은 그와 별개로 **회차 데드라인**(트리거 간격보다 짧게)을 두고 도달 시 잔여 회원사를 다음 회차로 미룬다. KIS가 전반적으로 느려도 락이 실행 중에 만료되지 않아 다중 기동에서 동시 실행이 생기지 않고, 이연분은 조회 창이 직전 영업일을 포함하므로 유실되지 않는다. 순회 시작 위치는 **트리거 시각에서 유도**한다(10분 회차 번호 mod 회원사 수) — 어떤 인스턴스가 락을 잡아도 같은 회차엔 같은 위치에서 시작하고 회차마다 회전하므로, 이연이 반복돼도 특정 회원사가 계속 뒤로 밀리지 않는다(인메모리 커서 없음 — 재기동 안전).
...
*KIS 수집 워커 명세 v0.6 — Redis 계약 v0.16·WS API v0.7·core-api 명세 v0.3과 정합. KIS 수치는 2026-07 공식 샘플 대조 기준이며 §9 항목은 실계정 재확인 대상.*
