...
- **확정되지 않는 호가 등록은 회전시킨다**: 같은 종목의 호가 등록 요청이 `MAX_DEPTH_ATTEMPTS`(3)회 확정되지 않으면 쿨다운(기본 60s) 동안 배정 대상에서 빼고 `depth.subscribe.cooldown`을 올린다. 그 슬롯은 다음 순위 방 종목이 받고, 쿨다운이 지나면 다시 경쟁한다 — 실패가 한 종목에 고착되지 않게 하되 재시도 자체를 포기하지도 않는다. 확정 ACK가 오면 실패 기록을 지운다. 호가는 선택 채널이라 이 실패로 세션을 재접속시키지는 않는다(quote 커버리지를 흔들지 않는다). 단 체결(quote) 수요가 슬롯을 되찾으면 단계와 무관하게 즉시 밀려난다 — depth는 quote 용량을 깎지 않는다. 같은 정비 주기 안에서는 **해지 프레임을 등록 프레임보다 먼저 보낸다** — 41건 한도가 꽉 찬 세션에서 해지 몫을 먼저 비워야 새 등록이 거절되지 않는다(체결 재구독 전환의 공백은 §2.5 REST 강등이 이미 받는다). **해지 전송이 실패한 등록은 로컬 추적을 지우지 않고 다음 정비에서 재시도한다** — 전송 성공 전에 지우면 그 연결이 살아 있는 동안 KIS 쪽 슬롯이 새서, 그 자리를 쓰려는 교체 등록이 영구히 거절된다(거절된 등록은 매 정비 재시도되지만 새는 슬롯은 회수 경로가 없다). 연결 자체가 죽은 경우는 이 경로가 아니라 재접속 리플레이가 정리한다 — KIS 등록은 연결 단위라 절단과 함께 사라진다.
