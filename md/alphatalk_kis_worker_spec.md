...
| `financials_sync` | OpenDART `list.json`(신규 정기공시 감지) → `fnlttSinglAcntAll.json` (`bsns_year`, `reprt_code` 11013/11012/11014/11011, `fs_div=CFS`→미존재 시 `OFS`) | 매일 06:00 (공시 시즌 증분) + 부분 실패 시 09·13·17시 재실행 | 신규 공시 기업 + **초기 백필**(행이 없는 종목, 회차당 상한 — §3.2) | `(corp_code, year, reprt_code)` |
...
- **`financials_sync` 초기 백필**: 신규 공시 감지만으로는 최초 배포 시점의 `financial_summary`가 영구히 비어 있다 — 공시 시즌 밖 배포면 대부분 종목이 한 행도 얻지 못하고, 이후에도 새 공시만 쌓인다(core-api 재무 API는 최근 3년 연간·분기를 요구한다). 그래서 매 회차 신규 공시 처리 뒤, **행이 하나도 없는 활성 종목**을 회차당 상한(`backfill-per-run`, 기본 100)만큼 골라 회사 단위 `list.json`(`corp_code` 지정)으로 최근 `backfill-years`(기본 3)년의 정기공시를 받아 **같은 파이프라인으로 처리**한다 — 접수일(`rcept_dt`)이 실제 값이라 `disclosed_at`도 정확하다. 조회 창은 **1년 단위로 쪼개 순차 요청**한다 — DART 공시검색의 기간 제한(corp 지정 시 1년)을 넘는 단일 요청은 `100`으로 거부되고, 그건 fatal 분류라 잡 전체가 실패한다. 며칠에 걸쳐 수렴한다(2,600종목 ÷ 100/일, 종목당 list ≤4콜 + 보고서 ≤12건 × ≤2콜). `corp_code` 매핑은 `dart_corp_map`에서 읽고, 매핑이 없는 종목은 건너뛰며(`batch.financials.backfill.unmapped`) 주 1회 `dart_corp_map` 갱신이 채우면 합류한다. **완료 판정은 행에서 추론하지 않고 마커 테이블 `financial_backfill(code PK, window_years, mapper_version, completed_at)`에 기록한다** — 행 기반 추론은 어떤 조건이든 오인 조합이 있다: "행 있음"은 공시 시즌 첫 배포에서 증분이 먼저 적재한 최신 행 하나에, "과거 연도 N개"는 다년 일괄 정정공시(감사의견 정정 등 실제 발생) N건에 속아 그 종목의 나머지 이력이 **관측 신호 없이 영구 누락**된다. 마커는 `completeBackfill`이 행 upsert와 **같은 트랜잭션**으로 기록하므로 중간에 죽으면 둘 다 없던 일이 되고, 종목 단위 원자성과 함께 "마커 없음 = 아직 백필 안 됨"이 정확해진다. 부수 효과로 정기공시가 원래 없는 종목(스팩 등)도 1회 시도 후 마커가 남아 매 회차 재시도되지 않는다. 마커에는 **완료 당시의 조회 지평(`window_years`)과 계정과목 매퍼 버전(`mapper_version` — 코드 상수 `FinancialAccountMapper.MAPPING_VERSION`)을 함께 기록**하고 둘 다 현재 값 이상인 마커만 완료로 인정한다 — `backfill-years`를 늘리면 좁은 창으로 완료된 종목이, **매핑 테이블을 고쳐 `MAPPING_VERSION`을 올리면 낡은 매퍼로 완료된 종목이** 자동으로 대상에 복귀한다. 매핑 실패(`unmapped`)는 결정적이라 마커 없이 재시도해도 낫지 않고(매 회차 헛돌며 스트릭만 채운다) 마커만 남기면 영구 확정된다 — 버전이 이 딜레마를 푼다. **매핑 테이블을 수정할 때는 반드시 `MAPPING_VERSION`을 올린다.** 재백필 완료 시 **이번에 조회했지만 더 이상 산출되지 않는 보고서의 기존 행을 같은 트랜잭션에서 삭제**한다 — 축소 방향 매퍼 수정(잘못된 매핑 제거)이나 정정으로 대체된 보고서의 낡은 행이 새 마커 아래 잔존하면 다시는 교정되지 않는다. 삭제 범위는 이번 백필이 관측한 (연도, 보고서)로 한정해 창 밖·미조회 행은 보존한다. 정기공시가 원래 없거나 상장 2년 미만이라 판정 연도 보고서가 없는 종목(스팩·신규 상장)은 수렴 후에도 매 회차 재시도되지만 하루 1회·소수라 수용한다. 회차당 상한은 정렬 prefix가 아니라 **날짜에서 유도한 회전 창**으로 뽑는다(`invest_opinion_sync`의 시각 유도 회전과 같은 패턴 — 상태 없음, 같은 날 재실행은 같은 창) — prefix로 뽑으면 영구 부적격 종목이 정렬 선두를 상한 개수만큼 점유하는 순간 나머지 전부가 영원히 백필되지 못한다. **쓰기는 종목 단위로 원자적**이다 — 보고서 하나라도 회차 안 재시도까지 실패하면 그 종목은 한 행도 쓰지 않고 통째로 다음 회차로 미루고(부분 적재는 커버리지를 만들며 공백을 영구히 숨길 수 있다), 저장 자체의 실패는 종목 실패로 흡수하지 않고 **잡 실패로 전파**한다(DB 장애를 종목별 실패로 오분류하면 남은 종목의 DART 조회만 태운다). 서킷브레이커 스트릭도 보고서가 아니라 **종목 단위로 1**을 센다 — 한 종목의 보고서 12건 실패가 스트릭을 채우면, 정렬 순서상 같은 종목이 매 회차 선두에 와 뒤 종목들이 영원히 백필되지 못한다.
