...
> **v0.9 (2026-08-17)**: worker-price 활성화 스위치를 `alphatalk.price.enabled`·`candle-enabled`·`minute-candle-enabled` 3개 플래그에서 **`KIS_ACCOUNTS` 설정 여부 하나로 통합**. 계정 배열이 비어 있으면 KIS 수집 평면(실시간 세션풀·REST 폴링·일봉·분봉·신선화 내부 API) 전체가 등록되지 않고(fail-closed), 계정이 있으면 전부 활성이다. 깨진 계정 JSON은 조용히 꺼지지 않고 기동 실패로 이어진다. `candle-sync-on-startup`(로컬 기동 백필 1회)만 별도 opt-in으로 남는다. v0.4의 "환경 차이는 워커 활성화 여부로 만든다"는 원칙은 유지하되 그 스위치가 플래그에서 계정 주입 여부로 바뀌었다.
