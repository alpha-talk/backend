...
- **확정되지 않는 호가 등록은 회전시킨다**: 같은 종목의 호가 등록이 `MAX_DEPTH_ATTEMPTS`(3)회 실패하면 쿨다운 동안 배정 대상에서 빼고 `depth.subscribe.cooldown`을 올린다. 실패는 **전송이 아니라 거절 ACK·실제 타임아웃에서만** 센다 — 전송 시점에 세면 마지막 요청의 ACK 창이 끝나기 전에 취소해 유효한 확정을 놓친다. 그 슬롯은 다음 순위 방 종목이 받는다. 확정 ACK가 오면 실패·쿨다운 기록을 지운다. 호가는 선택 채널이라 이 실패로 세션을 재접속시키지는 않는다(quote 커버리지를 흔들지 않는다).
