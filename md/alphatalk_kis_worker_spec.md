# Alpha Talk — KIS 수집 워커 명세 v0.10
**worker-price · worker-batch · `:kis-client` 공유 라이브러리 · 담당: 민균**

> **v0.10 (2026-08-18)**: worker-price 활성화 스위치를 `alphatalk.price.enabled`·`candle-enabled`·`minute-candle-enabled` 3개 플래그에서 **`KIS_ACCOUNTS` 설정 여부 하나로 통합**. 계정 배열이 비어 있으면 KIS 수집 평면(실시간 세션풀·REST 폴링·일봉·분봉·신선화 내부 API) 전체가 등록되지 않고(fail-closed), 계정이 있으면 전부 활성이다. 깨진 계정 JSON은 조용히 꺼지지 않고 기동 실패로 이어진다. `candle-sync-on-startup`(로컬 기동 백필 1회)만 별도 opt-in으로 남는다. v0.4의 "환경 차이는 워커 활성화 여부로 만든다"는 원칙은 유지하되 그 스위치가 플래그에서 계정 주입 여부로 바뀌었다.
...
- 목표 집합은 리컨실마다 산출한다. `rooms ∪ topN(quote)`이 C를 넘으면 **우선순위: ① 입장 방 ② quote refCount 내림차순**으로 자른다. 탈락분은 REST 폴링으로 강등한다(§2.5). ①의 구현: 신규 배정은 방 종목(refCount 내림차순)부터 처리하고, 세션이 가득 차 방 종목이 들어갈 자리가 없으면 **quote 전용 등록 하나를 선점**해 REST 폴링으로 밀어낸다(`tick.room.preempted` — 해지 유예 중인 심볼 우선 선정). **단 방 종목끼리는 선점하지 않는다** — 방 수요만으로 용량을 넘기면 늦게 온 방은 quote를 REST로 받고 depth 없이 지나간다. refCount가 더 높다고 기존 방의 등록을 빼앗으면 ⑴ 쫓겨난 방이 대신 depth를 잃는 제로섬 교체일 뿐이고, ⑵ refCount가 출렁일 때마다 방끼리 등록을 주고받는 플래핑이 생긴다(depth 슬롯 경쟁의 "활성 등록 우선"과 같은 원칙 — 안정 우선). 이 구간은 우선순위 조정이 아니라 **계정 증설로 푸는 문제**이며 `depth.symbols.dropped`가 그 신호다. 방이 아닌 quote끼리의 세부 순서(refCount 내림차순)는 아직 도착 순이다 — 초과분이 전부 REST로 받쳐지므로 후속으로 남긴다. 방 종목의 WS 배정을 보장하는 이유는 quote(REST 폴백 있음)와 달리 **depth는 WS 등록이 유일한 경로**이기 때문이다(§2.3 호가 확장).
...
- **등록 대상과 우선순위**: 리컨실마다 살아있는 게이트웨이의 `demand:room:{gwId}`를 합산해 refCount>0 종목을 **내림차순(동률은 코드 오름차순)**으로 정렬하고, 각 종목이 배정된 세션의 잔여 슬롯 안에서만 호가를 등록한다(§2.2). 슬롯이 없으면 등록하지 않고 `depth.symbols.dropped`로 노출한다 — 조용한 축소 금지. 방 수요가 사라지면 체결과 같은 해지 유예를 거쳐 해제된다. 슬롯이 모자랄 때의 배정 순서는 ① **유지 중이면서 여전히 방 수요가 있는 등록** ② **유예 홀드오버**(재입장·수요 블링크 플래핑 방지 — 신규 수요보다 앞선다) ③ **신규 방 수요**이며, 각 단계 안에서는 refCount 내림차순이다. ①과 ②의 "유지 중"은 **KIS가 확정한 등록**(성공 ACK, 또는 첫 ACK를 기다리는 유효기간 안의 요청)만 가리킨다 — 희망 배정을 기준으로 삼으면 등록이 거절되거나 응답이 없는 종목이 매 리컨실 슬롯을 선점해 뒤에 온 정상 방 종목이 영구히 굶는다(운영 기본인 TR 2건 구성에서는 세션당 호가 여유가 1건이라 한 종목의 실패가 그 계정의 모든 후속 방을 막는다). 확정된 등록은 신규 수요의 refCount가 더 높아도 빼앗기지 않는다(안정 우선).
- **확정되지 않는 호가 등록은 회전시킨다**: 같은 종목의 호가 등록이 `MAX_DEPTH_ATTEMPTS`(3)회 실패하면 쿨다운 동안 배정 대상에서 빼고 `depth.subscribe.cooldown`을 올린다. 실패는 **전송이 아니라 거절 ACK·실제 타임아웃에서만** 센다 — 전송 시점에 세면 마지막 요청의 ACK 창이 끝나기 전에 취소해 유효한 확정을 놓친다. 타임아웃으로 이미 실패 처리한 요청의 **거절 ACK가 늦게 도착하면 그 건은 흘려보낸다**(써버린 요청 수만큼, 연결이 순서를 보장하므로 다음 거절이 그 응답이다) — 세지 않으면 한 번의 실패가 두 번 계산되고, 재시도분의 추적까지 지워 그 뒤에 올 확정을 놓친다. **성공 ACK는 늦게 와도 그대로 받는다** — 늦든 아니든 KIS가 그 등록을 들고 있다는 직접 증거이고, 흘려보내면 정상 등록이 확정되지 못한 채 쿨다운으로 밀려난다. 흘려보내기 대기는 **그 등록 주기 안에서만 유효하다** — ⑴ 해지 성공 ACK를 받으면(순서 보장상 앞선 등록 요청들의 응답은 이미 지나갔다) 그 종목의 대기를 지운다. 남기면 쿨다운 뒤 새로 건 등록의 거절이 옛 응답으로 오인돼 실패 집계가 밀리고, 그 종목이 희소한 슬롯을 더 오래 붙든다. ⑵ 절단·재접속에서도 그 세션이 들고 있던 호가 등록(배정·등록 대기·해지 대기 전부)의 대기 상태를 버린다. 남기면 새 연결에서 온 유효한 거절을 옛 요청의 응답으로 오인해 실패 집계가 밀린다. 흘려보낸 횟수는 `depth.stale.ack.dropped`로 노출한다. 그 슬롯은 다음 순위 방 종목이 받는다. 확정 ACK가 오면 실패·쿨다운 기록을 지운다. 호가는 선택 채널이라 이 실패로 세션을 재접속시키지는 않는다(quote 커버리지를 흔들지 않는다).
- **쿨다운이 끝난 종목은 유지 등급으로 복귀한다**: 복귀분을 신규 수요로만 취급하면 용량이 계속 꽉 찬 구간에서 슬롯을 영영 되찾지 못한다 — 기본 구성은 세션당 호가 여유가 1건이라, 일시적 실패 한 번으로 **가장 refCount가 높은 방이 영구히 호가를 잃는다**. 그래서 복귀분은 유지 등급에서 refCount 순으로 경쟁하고, 자기보다 낮은 순위가 점유한 슬롯을 되찾는다(밀려난 쪽은 해지 유예를 거쳐 내려온다). 이 되찾기가 진동이 되지 않도록 **쿨다운은 실패할 때마다 2배로 늘린다**(기본 60s에서 시작해 최대 8단계). 계속 실패하는 종목은 빠르게 간격이 벌어져 점유자를 더 이상 흔들지 않고, 일시적으로 실패한 인기 방은 한 주기 안에 복귀한다. **복귀 자격은 그 방의 수요가 이어지는 동안만 유효하다** — 방이 비어 유예까지 지나면 자격을 버린다. 남겨두면 나중에 그 방이 다시 열릴 때 신규 수요를 복귀분으로 오인해, 그동안 정상적으로 호가를 받던 점유자를 밀어낸다(실패 격리 자체는 유지한다 — 그 종목의 등록이 실패하기 쉽다는 사실은 수요와 무관하다. 지우면 나가고 다시 들어오기를 반복하는 것만으로 지수 백오프가 매번 초기화돼, 등록이 되지 않는 종목이 세션의 유일한 호가 슬롯을 계속 잡아먹는다). 격리 단계는 마지막 실패로부터 **1시간** 동안 조용하면 소멸한다 — 오래전 한 번의 실패가 영구 가중치로 남지 않게 하고, 기록도 그 시점에 정리한다. 단 체결(quote) 수요가 슬롯을 되찾으면 단계와 무관하게 즉시 밀려난다 — depth는 quote 용량을 깎지 않는다. 같은 정비 주기 안에서는 **해지 프레임을 등록 프레임보다 먼저 보낸다** — 41건 한도가 꽉 찬 세션에서 해지 몫을 먼저 비워야 새 등록이 거절되지 않는다(체결 재구독 전환의 공백은 §2.5 REST 강등이 이미 받는다). **해지 전송이 실패한 등록은 로컬 추적을 지우지 않고 다음 정비에서 재시도한다** — 전송 성공 전에 지우면 그 연결이 살아 있는 동안 KIS 쪽 슬롯이 새서, 그 자리를 쓰려는 교체 등록이 영구히 거절된다(거절된 등록은 매 정비 재시도되지만 새는 슬롯은 회수 경로가 없다). 연결 자체가 죽은 경우는 이 경로가 아니라 재접속 리플레이가 정리한다 — KIS 등록은 연결 단위라 절단과 함께 사라진다.
- **해지는 전송이 아니라 ACK로 완료된다**: `sendText`가 끝난 것은 프레임을 보냈다는 뜻일 뿐 KIS가 수락했다는 뜻이 아니다. 그래서 전송 성공 시 등록을 **해지 대기(pending unsubscribe)로 옮기고**, 해지 성공 ACK를 받았을 때만 추적에서 지운다. 거절 ACK는 즉시, 무응답은 ACK 유효기간 뒤 재시도한다. **ACK가 해지 응답인지는 문자열과 로컬 상태 둘로 판정한다** — `msg1`이 `UNSUB…`로 시작하면 해지 응답이고, 그렇지 않은 **거절** 응답은 그 등록이 해지 대기 중이고 등록 대기가 아닐 때 해지 거절로 본다(`ALREADY IN UNSUBSCRIBE`처럼 예상 못 한 문구를 등록 ACK로 오분류해 그냥 버리면, 거절을 즉시 처리하지 못하고 유효기간마다 재시도하다 불필요한 재접속까지 간다). **성공 응답은 이렇게 재분류하지 않는다** — 해지를 보낸 뒤 늦게 도착한 등록 성공 ACK를 해지 완료로 오인해 추적을 지워버리고, 그 뒤에 오는 진짜 거절을 흘려버린다. 거절을 잘못 재분류하면 불필요한 해지 한 번으로 끝나지만(어차피 수요가 없는 등록이다), 성공을 잘못 재분류하면 슬롯 누수가 조용히 남는다. 재시도가 `MAX_UNSUBSCRIBE_ATTEMPTS`(3)를 넘으면 **그 세션을 재접속해 등록을 회수한다**(`kis.unsubscribe.abandoned`). 세 번 시도하고도 응답이 없거나 거절당하면 KIS가 그 등록을 들고 있는지 **로컬에서는 알 수 없다** — 들고 있다고 보면(확정 복원) 실제로는 해지된 종목이 데이터 없이 확정으로 남고, 없다고 보면(추적 삭제) 실제로는 남은 슬롯이 조용히 새면서 게이지에서도 사라진다. 어느 쪽으로 추측해도 한 갈래가 조용히 깨지므로 추측하지 않는다. **KIS 등록은 연결 단위**라 재접속하면 그 세션의 등록이 전부 사라지고 상태가 다시 확실해진다 — 배정분은 새 연결에서 재구독된다(기존 리플레이 경로). **연결이 끊긴 순간 그 세션의 배정 종목은 전부 강등해 REST가 받게 한다**(정비 주기는 **절단 흡수 → 접속·구독 시도 → 강등 판정** 순서로 돈다 — 판정을 앞에 두면 그 주기의 절단이나 접속 실패가 다음 주기에야 반영되고, 그 틈에 폴링 회차가 지나가면 다음 회차까지 30초가 통째로 빈다. 접속 시도 뒤에 판정해야 최초 접속 실패도 같은 주기에 잡힌다. 판정에 쓰는 시각은 주기 시작이 아니라 **판정 시점에 다시 읽고, 그 직전에 도착한 틱을 먼저 흡수한다** — 접속은 타임아웃까지 블로킹될 수 있어, 시작 시각으로 재면 접속에 걸린 시간만큼 침묵 판정이 늦고, 반대로 시각만 최신으로 당기고 그동안 도착한 틱을 흡수하지 않으면 멀쩡히 틱이 흐르는 종목을 강등한다 — 시각과 틱 상태는 같은 시점의 것이어야 한다). 판정은 절단 순간 한 번이 아니라 **연결이 없는 동안 매 주기** 그 세션의 배정 종목 전체에 적용한다 — 절단 뒤 새로 배정된 종목은 구독을 보낸 적이 없어 침묵 타이머조차 없으므로, 한 번만 표시하면 그 종목만 조용히 빠진다 — 재접속 백오프와 재구독 후 침묵 임계까지 합치면 수십 초가 비는데, 그동안 강등 목록에 없으면 WS도 REST도 없는 구간이 된다(현재 채널의 틱이 다시 오면 자동 해제된다). 이 규칙은 강제 재활용뿐 아니라 일반 절단에도 함께 적용한다 — 원인과 무관하게 "연결이 없는 동안은 REST가 받는다"가 하나의 규칙이다. 메트릭은 등록 단위가 아니라 **세션 재접속 1회당 1**로 센다(같은 세션의 여러 등록이 동시에 한도를 넘어도 재접속은 한 번이다). 유령 점유를 장부에 남기지 않으므로 용량 계산도 배정(assigned·depthAssigned)만 보면 된다.
- **침묵 감시는 ACK가 아니라 등록 요청 시점에 무장한다**: 확정 ACK를 기다려 타이머를 걸면, 등록이 계속 거절되는 종목은 `subscribedAt`이 없어 침묵 판정을 영원히 통과하지 못하고 REST 폴백에서도 빠진다 — 배정만 되고 아무 데이터도 오지 않는 사각지대다. 요청을 보낸 시각부터 재면 확정되지 않은 등록도 `silence-ms` 뒤 강등되어 §2.5가 받는다. 해지 대기 중에 수요가 돌아오면 대기를 취소하고 재등록한다.
- **ACK 유효기간은 재등록 조건일 뿐 추적 만료가 아니다**: 응답 없는 등록은 유효기간이 지나면 재전송하되 **추적에서 지우지 않는다.** 지우면 같은 주기에 수요까지 사라졌을 때 해지 대상 집합에서 함께 빠져, ACK가 유실·지연됐을 뿐 KIS에는 살아 있는 등록이 연결 수명 내내 슬롯을 점유한다(위와 같은 고갈 경로이며, 이쪽은 전송 실패조차 없어 로그도 남지 않는다). 추적을 유지하면 늦게 도착한 ACK도 확정으로 흡수되고, 수요가 사라진 등록은 정상적으로 해지된다. 방 수요 종목은 수요 정의(§2.1 — rooms ∪ watchlist)상 항상 체결 구독 대상에도 포함되므로, 호가만 있고 체결이 없는 종목은 없다.
...
- **현재 배정의 프레임만 버퍼에 넣는다**: 구분 전환·세션 재배정 구간에는 옛 등록의 해지 ACK를 기다리는 동안 옛 TR과 새 TR이 동시에 살아 있을 수 있고, 해지 재시도가 겹치면 수 초 이상 이어진다. 그때 옛 프레임이 새 호가를 덮어써도 depth 봉투에는 원천 시각도 `eventId`도 없어(스냅샷성) 클라이언트가 구분하지 못한다. 그래서 수신 시 **프레임을 보낸 세션이 그 종목의 현재 배정 세션인지**와 **`trId`가 현재 구분의 호가 TR인지**를 확인하고, 어긋나면 버리며 `depth.stale.dropped`를 올린다. 이 판정은 WS 프레임 스레드에서 일어나므로 정비 스레드의 상태를 직접 읽지 않고, 리컨실이 갱신하는 동시성 인덱스(`code → 배정 세션`)와 `market-div` 캐시만 읽는다(§2.3의 스레드 경계 원칙).
...
- **소스 API**: 당일은 `GET /uapi/domestic-stock/v1/quotations/inquire-time-itemchartprice` · TR `FHKST03010200` · `FID_INPUT_HOUR_1`(기준 시각) → **1콜 최근 30개 1분봉**. 과거 일자는 주식일별분봉조회 `GET /uapi/domestic-stock/v1/quotations/inquire-time-dailychartprice` · TR `FHKST03010230` — `FID_INPUT_DATE_1`(기준일)+`FID_INPUT_HOUR_1`(기준 시각) 이하의 봉을 **1콜 최대 120건** 최신→과거로 주고, 기준일 개장 전 시각에 도달하면 **같은 응답 안에서 직전 영업일로 이어진다**(계측 2026-08-09, §9.9 종결). 휴장일 날짜를 요청하면 에러가 아니라 직전 영업일 행이 오고, NXT 미지원 종목을 `UN`으로 물으면 엉뚱한 과거 날짜 행이 오므로 **`stck_bsop_date=요청일` 필터가 필수**다. `output1`은 요청일이 아닌 현재 시점 스냅샷이다. 원천은 KIS 공식 분봉이다 — 자체 틱 집계로 분봉을 만들지 않는다. §0의 "틱 비영속" 원칙은 유지된다: 틱을 대체하는 영속형이 봉이고, 그 봉의 값은 KIS가 준다.
...
- **과거 분봉 — 일 배치 확정**: `minute_candle_daily_sync`가 영업일 **20:05**(마지막 세션 20:00 종료 직후)에 그날 수요·조회 이력이 있던 종목의 당일 1분봉을 확정 적재한다(종목당 최대 25콜(`UN`)·14콜(`J`) — 장중 조회 경로가 이미 채운 구간은 스킵되어 실제 콜은 훨씬 적다). 콜드 종목의 그 이전 **7영업일**(설정 `minute-candle-backfill-days`, 0=off)은 **조회 트리거 백필**(`minute_candle_backfill`)이 채운다 — 신선화 내부 API가 호출될 때마다 worker-price가 **비동기로**(단일 워커 스레드, 조회 응답은 백필을 기다리지 않는다) 직전 7영업일 중 `minute_candle`에 행이 하나도 없는 날짜를 `FHKST03010230`으로 채운다. v0.3의 수요 0→1 전이 트리거는 폐기했다 — 분봉 소비의 원천이 REST 조회이고 신선화 트리거가 이미 조회 편승 지점이라, 게이트웨이 demand 구독 없이 같은 경계에서 처리한다. 규칙(계측 근거는 v0.8·§9.9):
    - **날짜 단위 원자 적재**: 하루를 마감 봉부터 개장 봉까지 끝까지 확보했을 때만 그날을 한 트랜잭션으로 적재한다. 부분 적재를 남기면 "행 있음 = 처리됨" 스킵 판정이 그 날짜를 영구히 건너뛴다. 페이지·시간 예산이 바닥나면 그날은 버리고 다음 트리거가 재시도한다.
    - **`stck_bsop_date=요청일` 필터**: 휴장일 요청(직전 영업일 행 반환)과 `UN` 미지원 응답(엉뚱한 날짜 행)을 모두 걸러낸다. 요청일 행이 0건이면 그날은 데이터 없음으로 스킵한다(휴장일).
    - **시장 구분은 날짜마다 새로 판정한다**: 모든 날짜를 `UN` 먼저 조회하고 **요청일 유효 행이 없으면 같은 날짜를 `J`로 재조회**한다 — 과거 날짜에는 당일 TR의 `acml_vol>0 && 전 행 0` 판정을 쓸 수 없고(`output1`이 현재 스냅샷이라), **현재 상태 캐시(`market-div:{code}`)로 과거를 판정하지도 않는다** — 창 안에서 NXT 제외가 있었던 종목을 현재 `J` 캐시로 조회하면 과거 NXT 일자의 장외 봉과 통합 거래대금을 조용히 잃고, `J` 행이 유효하게 적재되므로 재시도도 없다. 계측상 `UN`은 그 종목이 NXT였던 날짜의 행만 주므로(미지원 날짜는 엉뚱한 날짜 행 → 필터에 걸림) `UN` 우선은 모든 경우에 자기 교정된다. 비용은 `J` 종목의 날짜당 1콜 손실뿐이고 백필은 콜드 종목당 한 번이라 수용한다. 수집 창 밖 행(`J` 15:30 초과 등)은 적재하지 않는다.
    - **후방 페이징**: 마감 시각(`UN` 20:00 · `J` 15:30)부터 `FID_INPUT_HOUR_1 = 가장 오래된 행 − 1분`으로 내려간다. 완주 신호는 **요청일 이전 날짜 행의 출현 또는 개장 봉 도달**뿐이다 — 행을 이미 모은 뒤 완주 신호 없이 요청일 행이 끊기면(중간 빈 응답 등) 완주가 아니라 이상 응답으로 보고 그날을 버린다(부분 적재가 스킵 판정에 굳는 것 방지. 상장 첫날처럼 이전 데이터가 아예 없는 날은 재시도가 반복되지만 쿨다운·창 이탈로 바운드된다). 하루 최대 721봉 ÷ 120건 ≈ 7콜.
    - **거래대금 차분**: 하루 전체를 확보하므로 앵커는 자명하다 — 일 시작 누적 0 기준으로 오름차순 차분하고, 첫 봉의 분값은 자신의 누적값이다(`store.sumValueBefore` 불필요).
    - **중복 콜 방지·유량 격리**: 종목별 인메모리 쿨다운(기본 10분) + 분산 락 `lock:minute-backfill:{code}`(Redis 계약 v0.19)로 재조회·다중 인스턴스의 중복 KIS 콜을 막는다. 이미 채워진 날짜는 스킵하므로 재실행은 멱등이다. 백필의 로컬 리미터는 **봉 예산의 1/4**(`rate-factor×(1−poll-budget-factor)×0.25`)로 분리한다 — 동기 신선화(당일 read-through)와 같은 리미터를 쓰면 콜드 종목 요청이 몰릴 때 지연에 민감한 당일 경로를 굶긴다. 로컬 리미터 합이 내부 한도를 다소 넘는 것은 공용 gate(§1.3)가 전역 합산을 계정 한도 아래로 묶으므로 안전하다.
    - **오늘은 대상이 아니다**: 백필은 오늘(Seoul) 이전 날짜만 다룬다 — 당일은 신선화·일 확정의 영역이고, 워터마크·구분 기록 기계와 겹치지 않는다. 백필은 `minute:market-div:{code}:{date}`를 쓰지 않는다 — 날짜 단위 원자 적재라 "한 날짜 한 구분" 불변식이 구조적으로 성립한다.
...
| `minute_candle_backfill` | KIS `FHKST03010230` (§2.6 — 실행 주체는 price) | 조회 시 신선화 트리거에 편승(비동기·종목별 쿨다운 10분·날짜 단위 원자 적재) | 조회 종목 × 직전 7영업일 중 빈 날짜 | `(code,date,time)` |
...
| `investor_flow_daily` | KIS `GET .../inquire-investor` · TR `FHKST01010900` — **장마감 후 확정치** | 영업일 17:10 (+부분 실패 시 17:40·18:40 재실행 — §3.2 예외) | 전 종목 | `(code,date)` |
...
**업종 소유권**: `stock_master_sync`는 종목명·시장·상장주식수·상장일·활성 여부만 소유하고 **업종 필드를 갱신하지 않는다**(KIS 마스터의 업종 파싱·`sector` 적재는 제거). `sector` 카탈로그와 `stock_master.sector_code`·`dart_induty_code`는 `industry_sync`가 단독 소유한다 — 두 잡이 같은 컬럼을 쓰면 일 배치가 주 배치 결과를 덮는다. 두 잡은 스케줄러 풀이 1보다 크면 겹쳐 돌 수 있으므로(§3.2) `stock_master` 엔티티는 **바뀐 컬럼만 UPDATE한다**(`@DynamicUpdate`) — 전체 컬럼 UPDATE면 나중에 커밋하는 쪽이 읽어 둔 낡은 값으로 상대 소유 컬럼을 되돌린다.
...
| `financials_sync` | OpenDART `list.json`(신규 정기공시 감지) → `fnlttSinglAcntAll.json` (`bsns_year`, `reprt_code` 11013/11012/11014/11011, `fs_div=CFS`→미존재 시 `OFS`) | 매일 06:00 (공시 시즌 증분) + 부분 실패 시 09·13·17시 재실행 | 신규 공시 기업 + **초기 백필**(완료 마커 없는 종목, 회차당 상한 — §3.2) | `(corp_code, year, reprt_code)` |
...
- Spring `@Scheduled` + **ShedLock**(Redis) — 다중 기동에 안전하다. 잡 이력은 테이블 `batch_job_run(job, run_date, status, ok_count, fail_count, started_at, finished_at, error)`에 기록하고, 동일 `(job, run_date)` SUCCESS가 있으면 스킵한다(재실행 멱등). 모든 잡이 같은 `ThreadPoolTaskScheduler`를 쓰므로 `spring.task.scheduling.pool.size`를 1보다 크게 둔다(기본값 1이면 Spring이 이전 실행이 끝난 뒤에야 다음 크론을 잡아, 늘어진 DART 잡 하나가 그날의 KIS 잡을 통째로 건너뛰게 한다). 같은 잡의 동시 실행은 ShedLock이 인스턴스 안팎을 가리지 않고 막는다.
...
- 실패 종목은 잡 말미에 1회 재시도한다. 잔여 실패는 `fail_count`+로그로 남기고 다음 날 upsert로 자연 회복한다. **예외 — `valuation_daily`·`investor_flow_daily`**: 부분 실패 회차는 SUCCESS 대신 **FAILED로 기록**하고(ok/fail 카운트 보존 — `failCounted`), 재실행 크론(valuation 17:20·18:20, investor 17:40·18:40)이 FAILED 회차만 전 종목 재조회로 채운다(SUCCESS면 no-op, upsert 멱등). 사유는 다르다 — valuation은 날짜 키 스냅샷이라 다음 날 실행이 그날의 공백을 채우지 못하고, investor는 데이터 자체는 다음 날 응답(최근 ~30일 포함)이 채우지만 **전량 실패(KIS 스키마 드리프트로 파서가 전 종목에서 던지는 경우)가 SUCCESS로 굳으면 당일 재실행이 `start`의 SUCCESS 스킵에 막히고 FAILED 기반 알람도 침묵**한다. 마지막 재실행까지 실패가 남으면 FAILED로 확정돼 알람 대상이 된다. investor는 추가로 **영업일 전체 0행 적재도 FAILED**로 남긴다(`batch.investor.empty`) — 날짜 필드까지 개명되는 드리프트는 파서가 전 행을 패딩으로 오인해 예외 없이 빈 결과가 되는데, 활성 유니버스 전체가 0행인 영업일은 정상적으로 존재하지 않는다.
...
- **부분 성공은 성공이 아니다 — 날짜 키 스냅샷 잡의 공통 규칙**:
  - `stock_master_sync`는 한 시장이라도 재시도까지 실패했거나 **읽지 못한 행이 있으면**(파싱 불완전) FAILED로 남긴다. 그 회차의 유니버스는 부분이고(상장폐지 정리도 건너뛴다), SUCCESS로 굳히면 그날 재실행이 막혀 그 시장의 신규 상장 종목이 하루 종일 누락된다. **FAILED로 남기는 이상 같은 날 재실행 경로가 반드시 있어야 한다** — 09:00·10:00·12:00 재실행 크론을 둔다(SUCCESS면 파일을 내려받기 전에 스킵). 이게 없으면 08:00 일시 실패가 아래 하류 게이트와 맞물려 그날 `valuation_daily`·`investor_flow_daily`를 통째로 막아, 한 시장 누락을 막으려다 전 종목을 잃는다.
  - `valuation_daily`·`investor_flow_daily`는 실행 전에 **오늘 `stock_master_sync`의 완전 성공 행**(`SUCCESS` **이고** `fail_count = 0`)을 요구하고, 없으면 부분·낡은 유니버스를 처리하지 않고 FAILED로 남긴다. "행이 있는데 실패했으면"이 아니라 **"완전 성공이 있어야"**로 잡는 두 가지 이유: ⑴ 스테일 락으로 마스터 호출이 `runs.start` 전에 스킵되면 행 자체가 안 생기는데, 행 부재를 정상으로 보면 어제 유니버스로 그날을 굳혀 신규 상장 종목이 누락된다. ⑵ 롤링 배포로 이 규칙 이전 버전과 공존하는 동안 구버전이 `SUCCESS + fail_count>0`(부분 유니버스)을 남길 수 있는데, 상태만 보면 그걸 완전한 것으로 인정하게 된다.
  - **그 대신 마스터는 자기 부분 성공 행을 스스로 다시 연다** — `stock_master_sync`만 `startOrRepair`를 쓴다(완전 성공이 아니면 재시작, 완전 성공이면 스킵). 이게 없으면 `start`가 SUCCESS 행을 재시작하지 않아 위 ⑵의 행이 그날을 교착시킨다. 다른 잡의 `start` 의미는 그대로다 — `investor_flow_daily`처럼 잔여 실패를 SUCCESS로 기록하는 잡까지 재시작 대상이 되면 실패한 날마다 전 종목을 다시 조회한다.
  - `financials_sync`는 술어가 다르다 — **오늘 마스터 행이 있는데 SUCCESS가 아닐 때만 유예**한다(FAILED로 남겨 다음 재실행 크론이 다시 본다). 정규 회차가 06:00으로 마스터(08:00)보다 이르므로 "SUCCESS 필수"로 잡으면 매일 막힌다. 반대로 09:00·13:00·17:00 재실행은 마스터 뒤라, 유예하지 않으면 부분 유니버스로 공시를 걸러낸 채 SUCCESS를 굳혀 뒤 재실행이 스킵되고 창 끝 공시가 사라진다.
  - **선행 잡을 끈 환경은 막지 않는다.** 선행 잡 이름은 조립 시점에 `stock_master_sync` 빈이 있을 때만 주입한다(없으면 null → 검사 없음). 잡 안에서 이름을 고정하면 마스터를 끈 배포에서 밸류에이션·수급이 영원히 FAILED가 된다. 유니버스가 "비었는지"만 보면 한 시장만 적재된 콜드 부팅에서 나머지 시장이 그 날짜에 영구 누락된다. 행이 아예 없으면(마스터 잡을 끈 환경, 또는 마스터 크론 전인 06:00 `financials_sync`) 막지 않는다 — 없는 선행 잡을 요구하면 그 환경에서 영구히 멈춘다.
  - `financials_sync`는 재시도까지 실패한 공시가 남으면 FAILED로 남긴다. 감지 창이 `date-lookbackDays..date`로 굴러가므로 창 끝에서 실패한 공시는 **다음 날 창을 벗어나** 다시 감지되지 않는다. 09:00·13:00·17:00 재실행 크론이 같은 날 다시 시도하고(SUCCESS면 no-op), 그래도 남으면 알람 대상이다. 따라잡기 재확인 패스만으로는 부족하다 — 상시 떠 있는 워커는 기동 이벤트를 다시 받지 않는다.
- **`financials_sync` 초기 백필**: 신규 공시 감지만으로는 최초 배포 시점의 `financial_summary`가 영구히 비어 있다 — 공시 시즌 밖 배포면 대부분 종목이 한 행도 얻지 못하고, 이후에도 새 공시만 쌓인다(core-api 재무 API는 최근 3년 연간·분기를 요구한다). 그래서 매 회차 신규 공시 처리 뒤, **유효한 완료 마커가 없는 활성 종목**(아래 `financial_backfill` 판정)을 회차당 상한(`backfill-per-run`, 기본 100)만큼 골라 회사 단위 `list.json`(`corp_code` 지정)으로 최근 `backfill-years`(기본 3)년의 정기공시를 받아 **같은 파이프라인으로 처리**한다 — 접수일(`rcept_dt`)이 실제 값이라 `disclosed_at`도 정확하다. 조회 창은 **1년 단위로 쪼개 순차 요청**한다 — DART 공시검색의 기간 제한(corp 지정 시 1년)을 넘는 단일 요청은 `100`으로 거부되고, 그건 fatal 분류라 잡 전체가 실패한다. 며칠에 걸쳐 수렴한다(2,600종목 ÷ 100/일, 종목당 list ≤4콜 + 보고서 ≤12건 × ≤2콜). `corp_code` 매핑은 `dart_corp_map`에서 읽고, 매핑이 없는 종목은 건너뛰며(`batch.financials.backfill.unmapped`) 주 1회 `dart_corp_map` 갱신이 채우면 합류한다. **완료 판정은 행에서 추론하지 않고 마커 테이블 `financial_backfill(code PK, window_years, mapper_version, completed_at)`에 기록한다** — 행 기반 추론은 어떤 조건이든 오인 조합이 있다: "행 있음"은 공시 시즌 첫 배포에서 증분이 먼저 적재한 최신 행 하나에, "과거 연도 N개"는 다년 일괄 정정공시(감사의견 정정 등 실제 발생) N건에 속아 그 종목의 나머지 이력이 **관측 신호 없이 영구 누락**된다. 마커는 `completeBackfill`이 행 upsert와 **같은 트랜잭션**으로 기록하므로 중간에 죽으면 둘 다 없던 일이 되고, 종목 단위 원자성과 함께 "마커 없음 = 아직 백필 안 됨"이 정확해진다. 부수 효과로 정기공시가 원래 없는 종목(스팩 등)도 1회 시도 후 마커가 남아 매 회차 재시도되지 않는다. 마커에는 **완료 당시의 조회 지평(`window_years`)과 계정과목 매퍼 버전(`mapper_version` — 코드 상수 `FinancialAccountMapper.MAPPING_VERSION`)을 함께 기록**하고 둘 다 현재 값 이상인 마커만 완료로 인정한다 — `backfill-years`를 늘리면 좁은 창으로 완료된 종목이, **매핑 테이블을 고쳐 `MAPPING_VERSION`을 올리면 낡은 매퍼로 완료된 종목이** 자동으로 대상에 복귀한다. 매핑 실패(`unmapped`)는 결정적이라 마커 없이 재시도해도 낫지 않고(매 회차 헛돌며 스트릭만 채운다) 마커만 남기면 영구 확정된다 — 버전이 이 딜레마를 푼다. **매핑 테이블을 수정할 때는 반드시 `MAPPING_VERSION`을 올린다.** 재백필 완료 시 **이번에 조회했지만 더 이상 산출되지 않는 보고서의 기존 행을 같은 트랜잭션에서 삭제**한다 — 축소 방향 매퍼 수정(잘못된 매핑 제거)이나 정정으로 대체된 보고서의 낡은 행이 새 마커 아래 잔존하면 다시는 교정되지 않는다. 삭제 범위는 이번 백필이 관측한 (연도, 보고서)로 한정해 창 밖·미조회 행은 보존한다. 회차당 상한은 정렬 prefix가 아니라 **날짜에서 유도한 회전 창**으로 뽑는다(`invest_opinion_sync`의 시각 유도 회전과 같은 패턴 — 상태 없음, 같은 날 재실행은 같은 창) — prefix로 뽑으면 실패가 반복돼 마커를 얻지 못하는 종목이 정렬 선두를 상한 개수만큼 점유하는 순간 나머지 전부가 영원히 백필되지 못한다. **쓰기는 종목 단위로 원자적**이다 — 보고서 하나라도 회차 안 재시도까지 실패하면 그 종목은 한 행도 쓰지 않고 통째로 다음 회차로 미루고(부분 적재는 커버리지를 만들며 공백을 영구히 숨길 수 있다), 저장 자체의 실패는 종목 실패로 흡수하지 않고 **잡 실패로 전파**한다(DB 장애를 종목별 실패로 오분류하면 남은 종목의 DART 조회만 태운다). 서킷브레이커 스트릭도 보고서가 아니라 **종목 단위로 1**을 센다 — 한 종목의 보고서 12건 실패가 스트릭을 채우면, 같은 날 재실행이 같은 회전 창을 다시 밟아 뒤 종목들이 그날 내내 백필되지 못한다.
- **DART 운영 장애는 빠르게 실패한다**: 종목 단위로 흡수하는 실패는 대상별 데이터 문제(타임아웃·파싱 실패)까지다. 인증·한도·점검 상태(`010`·`011`·`012`·`020`·`021`·`100`·`101`·`800`·`900`·`901`)와 HTTP 429·5xx는 **즉시 잡을 실패**시킨다 — 전면 장애를 종목별 실패로 흡수하면 남은 전 대상 × 재시도 × 데드라인까지 공유 일일 호출량을 태운다. 상태 코드로 안 잡히는 장애(반복 타임아웃)는 **연속 실패 한도**(`failure-streak-limit`, 기본 5 — 성공·데이터 없음이 나오면 리셋)가 서킷브레이커로 끊고 FAILED로 남긴다(`batch.financials.breaker`). 재실행 크론이 같은 날 다시 시도한다.
  - 셋 다 `stock_master`가 비면 0건 SUCCESS가 아니라 FAILED로 남긴다. 성공으로 확정하면 그날 재실행이 막혀(`start`의 SUCCESS 스킵) 마스터가 뒤늦게 적재돼도 그날 지표가 비게 된다. 따라잡기가 `stock_master_sync`를 먼저 돌리는 것과 같은 이유이며, 순서만으로는 부족하다 — 잡별 크론 시각이 달라 기동 시점에 따라 마스터 창은 아직 안 지나고 뒤 잡 창만 지난 조합(예: 06:00~08:00 기동)이 생기기 때문에, 순서가 아니라 **데이터와 선행 회차 상태로** 막는다.
...
financial_backfill(code CHAR(6) PK, window_years SMALLINT, mapper_version SMALLINT, completed_at)
             -- financials_sync 초기 백필 완료 마커(§3.2) — 행 upsert와 같은 트랜잭션. window_years·mapper_version이 현재 값보다 낮으면 재백필
             -- mapper_version은 후속 changeSet(0009)이 DEFAULT 0으로 추가 — 버전 기록 전에 완료된 마커는 첫 버전(1)보다 낡아 자동 재백필된다.
             -- DEFAULT는 유지한다: 롤링 배포 중 구버전 워커의 마커 INSERT가 NOT NULL 위반으로 죽지 않아야 한다 (새 바이너리는 항상 명시 기록)
...
OpenDART 응답 상태는 **세 갈래로 나눈다**. ① 데이터 없음(`013`)·회사코드 목록에서 사라진 종목은 **배정 해제 대상**이다 — `sector_code`와 `dart_induty_code`를 함께 비운다. 둘 중 하나만 지우면 다음 실행이 낡은 `dart_induty_code`를 되살려 배정이 영원히 회수되지 않는다. ② 인증·요청제한·시스템 점검(`010`·`011`·`012`·`020`·`021`·`100`·`101`·`800`·`900`·`901`)과 HTTP 429·5xx는 예외로 전파해 잡을 실패시킨다(`financials_sync`와 같은 분류 — §3.2). ③ 네트워크 오류·타임아웃·응답 파싱 실패는 종목 단위 실패로 흡수하고 1회 재시도한다 — 2,600건을 순차 호출하는 잡에서 한 건의 연결 끊김이 전체 회차를 버리면 안 된다. 다만 상태 코드로 안 잡히는 전면 장애(반복 타임아웃)는 **연속 실패 한도**(`failure-streak-limit`, 기본 5 — 성공·데이터 없음이 나오면 리셋)가 끊고, 회차 전체에는 **실행 데드라인**(`deadline`, 기본 90분 — ShedLock `lockAtMostFor` 2시간에서 마지막 호출·저장 여유 15분을 뺀 105분이 상한이며 넘는 설정은 기동을 거부한다, 호출 사이에서 검사)을 둔다. 둘 중 하나에 닿으면 아무것도 저장하지 않고 FAILED로 남긴다(`batch.industry.breaker`·`batch.industry.deadline`) — 요청 타임아웃이 60초라 데드라인 없이는 DART가 멈춘 날 2회 × 2,600건 × 60초가 스케줄러를 붙잡는다. 연속 실패 한도는 **1차 순회에서만** 검사한다(호출 직전마다, 그리고 1차 순회가 끝난 직후 한 번 더 — 마지막 호출들이 한도를 채운 경우를 놓치지 않기 위해) — 재시도 순회는 실패 종목만 연달아 호출하므로 산발적인 종목별 실패(허용치 5% 안의 깨진 응답 몇 건)가 연속 실패로 보여 정상 회차를 버리게 된다. 재시도 순회는 데드라인이 막고, 잔여 실패의 판정은 `max-failure-ratio`가 맡는다. 데드라인은 호출 직전에만 검사하므로 마지막 호출이 데드라인을 넘겨 끝난 회차는 그대로 저장한다 — 이미 받은 결과를 버려도 시간은 돌아오지 않는다.
...
| 메트릭 | `batch.industry.synced` · `batch.industry.failed` · `batch.industry.oversized` · `batch.industry.deadline` · `batch.industry.breaker` |
...
메트릭: `kis_ws_sessions{state}` · `kis_subscribed_symbols` · `demand_symbols` · `degraded_symbols` · `tick_in_rate`/`quote_publish_rate` · `conflation_lag_ms` · `pingpong_miss` · `rest_call_rate{keyId}` · `rest_throttled` · `token_refresh_total` · `batch_job_duration/fail{job}`. 로그는 구조화 JSON으로 남기고 appkey/token은 마스킹한다. 프레임 원문은 DEBUG+샘플링으로만 남긴다. 실시간 메트릭(§2.3): `tick.div.resubscribed`(구분 기록 갱신으로 재구독) · `tick.silence.degraded`(등록됐는데 체결 틱이 없어 REST 폴링으로 넘긴 종목) · `tick.market.div{div}`(실시간이 확정한 구분 — `UN`만 나온다). **`tick.silence.degraded`는 발생 즉시 알람** — 실시간 경로가 그 종목을 못 받고 있다는 뜻이다. 호가 메트릭(§2.3): `depth.in` · `depth.published` · `depth.symbols`(**KIS가 호가를 들고 있는 종목 수** = 확정 + 해지 대기 기준의 서로 다른 종목 — 희망 배정이 아니라 실제 등록만 세고, 해지는 성공 ACK로 완료되므로 대기분을 빼면 누수가 가려진다. 구분 전환·세션 재배정 중에는 한 종목이 옛 TR·새 TR 두 등록을, 심지어 서로 다른 세션에 걸쳐 가질 수 있어 **풀 전체에서 종목 단위로 중복을 제거한다**(세션별로 세어 합하면 옮겨가는 종목이 두 번 잡힌다). `kis_subscribed_symbols`도 같은 기준이다) · `kis.unsubscribe.abandoned`(해지 재시도 한도 초과로 세션을 재접속시킨 횟수 — 잦으면 KIS 해지 응답 규약을 재확인한다, §9.16) · `depth.symbols.dropped`(잔여 슬롯 부족·쿨다운으로 등록하지 못한 방 수요 종목 — 지속되면 계정 추가 신호) · `depth.subscribe.cooldown`(확정되지 않아 슬롯을 넘긴 종목) · `depth.stale.ack.dropped`(타임아웃 처리한 요청의 늦은 거절을 흘려보낸 횟수 — 꾸준히 오르면 ACK 유효기간이 KIS 응답 지연보다 짧다는 신호) · `depth.stale.dropped`(현재 배정과 어긋나 버린 호가 프레임 — 전환 구간에만 잠깐 오르고 지속되면 해지·재구독이 수렴하지 않는다는 신호) · `tick.room.preempted`(방 수요가 가득 찬 세션에서 quote 전용 등록을 밀어낸 횟수 — §2.2). 분봉 메트릭(§2.6): `minute.candle.refresh` · `minute.candle.upsert.retry` · `minute.candle.empty.complete`(`output2`가 빈 채로 완주) · `minute.candle.zero.page`(전 행 0인 페이지 — 시장 구분 오판·KIS 이상) · `minute.candle.value.regressed`(누적 거래대금 역행 = 시장 구분 혼입 의심) · `minute.candle.market.div{div}`(날짜별 판별 결과 분포) · `minute.candle.div.unknown`(그날 구분 기록 없이 적재분만 있어 갱신을 건너뛴 횟수 — Redis 유실 신호). 알람: WS 세션 전멸 5분, 장중 tick_in=0, 배치 실패, throttled 급증, `candle_sync_aborted`(유니버스 조회 실패로 일봉 회차 중단), `candle_sync_failed` 지속(재시도 회차까지 남는 종목 실패 — 둘 다 §2.6), **`minute.candle.value.regressed` 발생 즉시**(앵커 오염은 조용히 번진다).
...
9. ~~**분봉 API 계측(§2.6)**~~ — **종결**(2026-08-09 실계정 계측, §2.6·v0.8에 반영). 결과: `FHKST03010200`은 1콜 30건(기구현과 일치). `FHKST03010230`은 (a) 1콜 **최대 120건**, 최신→과거 정렬, 기준일 개장 전 시각에 도달하면 같은 응답 안에서 직전 영업일로 이어진다. (b) 시각은 **봉 시작**(`stck_cntg_hour` `HHmmss` — 09:00 개장 동시호가 봉이 `090000`, 마감 봉 `J` `153000`·`UN` `195900`). (c) `cntg_vol`은 **분값**, `acml_tr_pbmn`은 **당일 누적**(차분 필요 — 검증: 005930 20260807 090100 봉 225,292,604,750−196,613,623,750 ≈ 122,060주×235,250원). (d) 휴장일 날짜 요청은 `rt_cd=0`으로 직전 영업일 행을 반환하고, NXT 미지원 종목 `UN` 요청은 엉뚱한 과거 날짜 행을 반환한다(047040 20260807 요청 → 20260211 행) — 요청일 필터 필수. (e) `output1`은 요청일이 아닌 현재 시점 스냅샷. (f) 과거 범위는 최소 1달 전까지 정상(보존 30일·백필 7영업일에 충분).
...
16. **해지 ACK의 실제 문자열(§2.3)** — 등록 성공 응답이 `body.msg1="SUBSCRIBE SUCCESS"`인 것은 공식 예제로 확인했고(open-trading-api `chk_ccnl_krx.py`), 해지 응답은 `UNSUB…` 접두사로 구분한다고 가정하고 구현했다. 실계정 캡처로 해지 성공·거절 응답의 `msg1`·`msg_cd`·`rt_cd`를 확정한다. 가정이 틀리면 해지 ACK가 등록 ACK로 분류되어 해지 대기가 유효기간마다 재전송되다 한도에서 세션 재접속으로 이어진다(`kis.unsubscribe.abandoned`가 신호 — 등록 자체는 실제로 해지되므로 데이터 오염은 없고 재접속 비용만 든다).

  같은 캡처에서 **해지 대기 중 수요가 돌아오는 경합**도 함께 확정한다. 현재는 그 등록을 해지 대기에서 빼고 곧바로 재등록한다 — 한 연결 위에서는 해지→등록 순서가 보존되므로 최종 상태는 "등록됨"이 되고, 늦게 오는 해지 ACK는 `confirmed`를 건드리지 않는다. 다만 KIS가 **해지를 거절해 놓고 뒤이은 재등록도 "이미 등록됨"으로 거절**한다면(두 응답이 서로 모순되므로 일관된 서버에서는 나오지 않는 조합이다) 그 등록의 추적이 사라져 연결 수명 동안 슬롯 하나가 샌다. 재등록을 보류하고 해지 결과를 기다리는 방식이 이 조합까지 막지만, 보류 구간의 침묵 무장·거절 시 확정 승격까지 함께 얽히므로 **실제 응답 규약을 확인한 뒤에 정한다**. 그때까지의 완화책은 이미 있는 재접속 경로다(어떤 절단·재활용에서도 슬롯은 전부 회수된다).
---

*KIS 수집 워커 명세 v0.9 — Redis 계약 v0.22·WS API v0.9·core-api 명세 v0.5와 정합. KIS 수치는 2026-07 공식 샘플 대조 기준이며 §9 항목은 실계정 재확인 대상.*
