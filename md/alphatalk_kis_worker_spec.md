...
- **완주 워터마크**: "15:30까지 조회 완료" 표식은 Redis `minute:through:{code}:{date}`(TTL 2일, Redis 계약 v0.13)에 둔다 — 프로세스 로컬이면 리더 전환·재기동 때마다 거래정지·희소 종목을 다시 전 구간 조회하게 된다. 완주 판정은 `15:30 봉 존재 ∨ 워터마크 ≥ 15:30`이다.
...
| `minute_candle_daily_sync` | KIS `FHKST03010200` (§2.6 — 실행 주체는 price. **완주 판정 = 15:30 봉 존재 또는 15:30까지 조회 완료 워터마크**(희소·거래정지 종목은 마지막 체결이 일러도 완주). 종목별 시도 3회·백오프, 예외·경합은 다음 회차 재시도) | 영업일 16:00 + 17·18·19시 15분 재시도 — **회차마다 대상을 재계산**해 미완주가 없으면 no-op | 당일 수요·조회 이력 종목(`symbols ∪ minute_candle` 당일 적재 종목) | `(code,date,time)` |
