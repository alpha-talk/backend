...
- **`financials_sync` 초기 백필**: 신규 공시 감지만으로는 최초 배포 시점의 `financial_summary`가 영구히 비어 있다 — 공시 시즌 밖 배포면 대부분 종목이 한 행도 얻지 못하고, 이후에도 새 공시만 쌓인다(core-api 재무 API는 최근 3년 연간·분기를 요구한다). 그래서 매 회차 신규 공시 처리 뒤, **행이 하나도 없는 활성 종목**을 회차당 상한(`backfill-per-run`, 기본 100)만큼 골라 회사 단위 `list.json`(`corp_code` 지정)으로 최근 `backfill-years`(기본 3)년의 정기공시를 받아 **같은 파이프라인으로 처리**한다 — 접수일(`rcept_dt`)이 실제 값이라 `disclosed_at`도 정확하다. 조회 창은 **1년 단위로 쪼개 순차 요청**한다 — DART 공시검색의 기간 제한(corp 지정 시 1년)을 넘는 단일 요청은 `100`으로 거부되고, 그건 fatal 분류라 잡 전체가 실패한다. 며칠에 걸쳐 수렴한다(2,600종목 ÷ 100/일, 종목당 list ≤4콜 + 보고서 ≤12건 × ≤2콜). `corp_code` 매핑은 `dart_corp_map`에서 읽고, 매핑이 없는 종목은 건너뛰며(`batch.financials.backfill.unmapped`) 주 1회 `dart_corp_map` 갱신이 채우면 합류한다. **대상 산출은 "행 없음"이 아니라 "과거 커버리지 없음" 기준이다** — 커버리지는 `year <= 당해-1` 행이 **서로 다른 연도 2개 이상**일 때만 인정한다. 행 유무로 판정하면 공시 시즌의 첫 배포에서 증분 처리가 최신 행 하나를 먼저 적재해 그 종목이 백필 대상에서 영구히 빠지고(스냅샷 시점을 옮겨도 다음 날 재발), 단일 과거 행으로 판정하면 오래된 정정공시 하나가 같은 오인을 만든다. 완료된 백필은 3개 연도를 남기므로 2개 연도 조건으로 구분되고, 커버리지 기준이라 상태 테이블도 필요 없다. `backfill-years`는 판정 연도 수(2) 이상이어야 한다 — 작으면 백필 창이 판정을 채울 보고서를 못 잡아 같은 종목을 매 회차 재시도한다(기동 시 fail-fast). 정기공시가 원래 없거나 상장 2년 미만이라 판정 연도 보고서가 없는 종목(스팩·신규 상장)은 수렴 후에도 매 회차 재시도되지만 하루 1회·소수라 수용한다. 회차당 상한은 정렬 prefix가 아니라 **날짜에서 유도한 회전 창**으로 뽑는다(`invest_opinion_sync`의 시각 유도 회전과 같은 패턴 — 상태 없음, 같은 날 재실행은 같은 창) — prefix로 뽑으면 영구 부적격 종목이 정렬 선두를 상한 개수만큼 점유하는 순간 나머지 전부가 영원히 백필되지 못한다. **쓰기는 종목 단위로 원자적**이다 — 보고서 하나라도 회차 안 재시도까지 실패하면 그 종목은 한 행도 쓰지 않고 통째로 다음 회차로 미루고(부분 적재는 커버리지를 만들며 공백을 영구히 숨길 수 있다), 저장 자체의 실패는 종목 실패로 흡수하지 않고 **잡 실패로 전파**한다(DB 장애를 종목별 실패로 오분류하면 남은 종목의 DART 조회만 태운다). 서킷브레이커 스트릭도 보고서가 아니라 **종목 단위로 1**을 센다 — 한 종목의 보고서 12건 실패가 스트릭을 채우면, 정렬 순서상 같은 종목이 매 회차 선두에 와 뒤 종목들이 영원히 백필되지 못한다.
