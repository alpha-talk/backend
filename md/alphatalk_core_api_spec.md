...
- 분봉 데이터가 없는 구간(수집 시작 전·보존 30일 초과·모의 환경의 과거 일자)은 있는 만큼 반환하고 `hasMoreBefore:false`를 준다. 과거 7영업일은 일 배치·수요 전이 백필이 채우므로(워커 명세 §2.6) 첫 조회 직후에는 과거 구간이 비어 있다가 재조회에서 채워질 수 있다 — 빈 구간은 오류가 아니다.
