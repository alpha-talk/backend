...
Set을 먼저 맞춰야 발행 직후 재접속한 세션도 같은 관심목록을 해소한다. 이미 접속한 세션은 발행된 diff를 멱등 적용한다. 저장소는 트랜잭션 안에서 결과만 판정해 돌려주고 Redis는 건드리지 않는다 — 미러 갱신과 발행은 서비스가 그 뒤에 하므로 항상 커밋 이후다.

**Set 갱신과 발행은 멱등 재요청에도 매번 반복한다.** DB 커밋 뒤 Set 갱신이나 발행에서 실패하면 클라는 같은 PUT/DELETE를 재요청한다. 재요청은 Set을 다시 맞춘 뒤 같은 diff를 재발행해 접속 중인 게이트웨이까지 복구한다. 게이트웨이의 diff 적용은 멱등이므로 중복 발행은 상태를 중복 반영하지 않는다.
