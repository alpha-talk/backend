...
Set을 먼저 맞춰야 발행 직후 재접속한 세션도 같은 관심목록을 해소한다. 이미 접속한 세션은 발행된 diff를 멱등 적용한다. 저장소는 첫 트랜잭션에서 DB 변경을 커밋하고 Redis는 건드리지 않는다. 그 뒤 동기화 트랜잭션이 같은 사용자 잠금을 다시 잡아 현재 DB 상태를 확인하고, 잠금을 유지한 채 Set과 diff를 같은 상태로 맞춘다. 따라서 같은 종목의 PUT과 DELETE가 겹쳐도 늦게 실행된 Redis 작업이 더 최신 DB 상태를 과거 상태로 덮어쓰지 않는다.

**Set 갱신과 발행은 멱등 재요청에도 매번 반복한다.** DB 커밋 뒤 Set 갱신이나 발행에서 실패하면 요청은 500을 반환하지만 DB 변경은 유지된다. 클라는 같은 PUT/DELETE를 재요청하고, 재요청은 현재 DB 상태로 Set을 다시 맞춘 뒤 diff를 재발행해 접속 중인 게이트웨이까지 복구한다. 최초 PUT이 500 뒤 재요청되면 DB에는 이미 행이 있으므로 재요청의 정상 응답은 200이다. 게이트웨이의 diff 적용은 멱등이므로 중복 발행은 상태를 중복 반영하지 않는다.
