# Alpha Talk — Redis 계약 (`:contracts`) v0.23

> v0.23 (2026-08-18): **기사 적재도 다이제스트와 같은 단일 실행 규약으로 통일한다(§2.1).** 기사 경로는 `SETNX seen:ingest` 마커를 먼저 기록하고 XADD 실패 시 마커를 지우는 보상 방식이었는데, 마커 기록과 보상이 같은 Redis라 보상이 필요한 순간(마커 성공 직후의 과도기 장애)에는 보상도 함께 실패하기 쉽다 — 그 순서로 죽으면 기사가 마커 TTL 7일 동안 유실된다. 이제 모든 `queue:ingest` 적재(기사·다이제스트)가 마커 확인 → `XADD` → 마커 기록의 Redis 단일 실행 하나를 공유한다. 실패는 "마커 없음(다음 폴링이 재시도)" 또는 "중복 XADD(소비 측 `sourceId` 멱등이 흡수)"로만 떨어지고, 마커만 남고 엔트리가 유실되는 순서는 사라진다. 와이어 스키마·키 추가 없음.
...
### 1.3 수요 신호 채널 `demand:updated` — 게이트웨이 → worker-price (v0.21)
...
```json
{ "gwId": "a3f9c2b1d4e6", "ts": 1719600000000 }
```

- 의미는 "이 게이트웨이의 수요 집합이 방금 바뀌었다"뿐이다. **종목 단위 정보는 싣지 않는다**(v0.21) — 구독자가 payload를 파싱하지 않고 수신 자체만 트리거로 쓰므로, 종목별 N건 발행은 왕복 낭비였다.
- 게이트웨이는 **어떤 종목이든 참조수 0↔1 전이가 하나라도 있었던 sync에서만 1건** 발행한다(모든 증감마다·모든 sync마다 발행하지 않는다). 전이 없는 하트비트 sync에서 발행하면 worker-price가 5초마다 상시 리컨실하게 되어 트리거 설계가 무너진다.
- best-effort다. worker-price는 이 메시지를 **리컨실 트리거**로만 쓰고, 목표 종목 집합은 항상 §3의 해시 합산으로 재계산한다(payload를 신뢰해 부분 갱신하지 않는다). 유실은 주기 전체 리컨실이 자기치유한다 — 주기는 worker-price의 구독 해제 유예(`removal-grace-ms`, 기본 30s)보다 짧아야 하며(기본 10s, 기동 시 검증), 그래야 수요를 한 번 놓쳐도 KIS 구독이 실제로 끊기기 전에 복구된다.
...
| 필드 | 예 | 설명 |
|---|---|---|
| `source` | `"yna"` `"hankyung"` `"dart"` | 출처 |
| `sourceId` | `"a1b2c3"` | 출처 고유 ID — **중복 제거 키** |
| `type` | `"news"` `"report"` `"disclosure"` `"digest"` | 원본 종류 — `digest`는 일일 브리핑 잡(§2.3) |
| `codes` | `"005930,000660"` | 영향 종목 **후보**(콤마구분, 다중 가능). **공란 허용** — 소스가 종목을 아는 경우(DART 공시 등)만 싣고 텍스트 사전 매칭은 하지 않는다(v0.6). 관련 종목 확정·발견은 llm-worker LLM이 전담(뉴스 워커 명세 §2.4·§3.6). `digest` 타입만 1개 필수(§2.3) |
| `title` | `"..."` | 원문 제목 |
| `url` | `"https://..."` | 원문 링크 |
| `body` | `"..."` | 원문 본문/발췌(선택) |
...
적재는 종류와 무관하게 `seen:ingest:{sourceId}` 확인 → `XADD queue:ingest` → 마커 기록을 Redis 단일 실행으로 직렬화한다(v0.23 — 기사·다이제스트 공통). XADD보다 마커를 먼저 기록하지 않는다. XADD 실패에는 마커가 남지 않아 다음 폴링·재조정이 다시 시도하고, 마커 기록 실패 뒤 중복 XADD는 소비 측 멱등(§2.2)으로 흡수한다. 이 단일 실행은 `seen:ingest:*`와 `queue:ingest` 두 키를 한 스크립트에서 만지므로 **단일 Redis(비클러스터) 전제**다 — 클러스터 전환 시 CROSSSLOT으로 깨지며, 해시 태그로 같은 슬롯을 보장하거나 2단계 적재로 되돌리고 유실 창을 다시 검토해야 한다.
...
적재는 §2.1의 단일 실행 규약을 그대로 쓴다 — `sourceId`가 `digest:{code}:{date}`라 하루 1회로 고정된다는 점만 다르다.
