...
이 문서는 개발 환경에서 외부 임베딩 API 비용 없이 `worker-llm`의 뉴스 클러스터링을 검증하는 방법을 설명한다.

## 1. 권장 구성

로컬 프로파일의 기본 구성은 다음과 같다.
...
`LLM_PROVIDER=codex-cli`로 요약·판정을 Codex CLI 구독으로 전환할 수 있다. Claude CLI와 Codex CLI는 임베딩 API를 제공하지 않으므로 임베딩은 별도 로컬 서버가 담당한다.

`worker-llm` 안에서 모델을 직접 로딩하는 방식은 현재 구현하지 않는다. `EmbeddingClient` 포트의 `rest` 구현이 같은 호스트 또는 내부망의 Ollama를 호출한다. Ollama를 같은 서버에서 실행하면 기사 텍스트가 외부 임베딩 API로 전송되지 않는다.
...
Redis·PostgreSQL·Ollama를 실행한 상태에서 로컬 프로파일을 활성화한다.
...
`EMBEDDING_API_KEY=ollama`는 실제 비밀키가 아니다. 애플리케이션이 `provider=rest`에서 빈 키를 허용하지 않기 때문에 사용하는 더미 값이며 로컬 Ollama는 이 값을 인증에 사용하지 않는다.

## 7. Codex CLI와 함께 worker-llm 실행

LLM provider만 `codex-cli`로 변경한다.
...
`RestEmbeddingClient`가 `EMBEDDING_BASE_URL` 뒤에 `/v1/embeddings`를 붙인다. 따라서 `EMBEDDING_BASE_URL=http://localhost:11434/v1`로 설정하면 잘못된 `/v1/v1/embeddings` 주소를 호출한다.

로컬 프로파일은 fake 임베딩을 허용한다. `EMBEDDING_PROVIDER`가 정확히 `rest`가 아니면 알 수 없는 provider 값도 fake 임베딩으로 처리될 수 있으므로 시작 로그에서 실제 클라이언트를 확인한다.
...
Fake 임베딩은 문자열 토큰 해시를 1024차원 벡터로 만드는 테스트 구현이다. 비용과 모델 설치가 필요 없지만 문장 의미를 제대로 비교하지 못하므로 다음 용도로만 사용한다.
...
서로 다른 임베딩 모델이 만든 벡터를 같은 클러스터 검색 공간에 섞으면 코사인 유사도에 의미가 없다.
...
애플리케이션이 받은 벡터 길이가 설정한 `dimension`과 다르다는 뜻이다.
...
런타임 호출 실패 시 worker는 해당 기사를 임베딩 없이 신규 클러스터로 처리하고 경고 로그를 남긴다. 파이프라인은 계속 진행되지만 동일 사건이 여러 클러스터로 쪼개질 수 있으므로 반복되는 실패를 정상 상태로 방치하지 않는다.
