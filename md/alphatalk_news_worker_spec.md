...
| 3차 | — | — | 다이제스트에만 반영 |
  ...
  **두 상한은 업종에서 유추한 fan-out에만 적용한다** — LLM이 이름으로 직접 지목한 종목(`stocks[].relevant`)은 3차 억제에서도 계속 발행한다. 유추 배달은 규모가 커지면 노이즈지만 직접 지목은 기사가 그 종목을 다룬다는 뜻이라 상한의 대상이 아니다. 따라서 실시간 발행 총량은 `fanout-hard-cap` + 직접 지목 종목 수다.
