...
- MVP RSS는 연합뉴스·한국경제·동아일보·조선일보의 경제·산업·금융 섹션만 쓴다. 매일경제·뉴시스는 기본 목록에서 뺐고, 주식 관련성이 낮은 사회·정치 섹션과 경향신문·한겨레·서울경제 피드도 제외한다. 본문 fetch 허용 호스트(worker-llm `allowed-host-suffixes`)는 피드를 다시 넓힐 때를 대비해 더 넓게 유지한다 — 허용 목록에만 있고 피드에 없는 언론사는 기사가 들어오지 않아 무해하다.
...
| exact 중복 | `seen:ingest:{sourceId}` 확인 → `XADD` → 마커 기록(TTL 7일)을 Redis 단일 실행으로 처리, 마커가 있으면 skip — 같은 기사 재수집 흡수([redis_contract](redis_contract.md) §2.1). 마커를 XADD보다 먼저 기록하지 않으므로 적재 실패가 유실로 남지 않는다 |
...
- **v0.6 결정: 수집 측 텍스트 매칭(종목명 사전·매크로 키워드)을 코드 레벨에서 제거했다.** 수집 단계는 기사를 탈락시키거나 종목을 확정하지 않고, 매크로 `macroHint`도 적재하지 않는다. `stock_alias`는 수집 사전 매핑이 아니라 llm-worker의 후보 밖 `DIRECT` 판정 근거 검증에만 쓴다(§3.4·§5).
...
- `relevant=false`와 `relation=INDIRECT`인 종목은 개별 종목 연결에서 제외한다. 소스가 부여한 후보는 `DIRECT` 판정이면 채택하고, 후보 밖 발견 종목은 `evidence`가 실제 입력 문자열이며 정식명 또는 `stock_alias`를 포함하는지 결정론적으로 재검증한다. confidence만으로 직접 관련성을 승인하지 않는다.
...
- `positives/negatives`에는 선별된 종목 직접 뉴스만 담고, 섹터·시장 요인은 `sectorIssues`/`marketIssues` 버킷으로 분리한다. `includedCounts`는 실제 LLM 입력·payload에 포함된 클러스터 수이며 `pipelineVersion=2`로 이 유계 선별 규칙을 식별한다.
...
└─ queue/       IngestQueue(포트) · RedisIngestQueue(마커 확인→XADD→마커 기록 단일 실행 — exact 중복 제거 포함)
...
| 5 | RSS 이용조건 확정 | 현재 4개 언론사의 공식 RSS를 사용한다. 상업 출시 전 언론사별 이용조건·제휴 필요 여부를 최종 확인 |
