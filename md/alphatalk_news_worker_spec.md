...
- **`date`는 멱등 키이지 내용의 기준 시각이 아니다.** 지연 소비·기동 보충으로 잡이 날짜보다 늦게 돌면 리서치는 실행 시점 기준이라 그 사이 정보(예: 밤사이 미국 마감)가 포함될 수 있다 — 이는 오류가 아니라 의도된 의미론이다. 검색 결과의 발행 시각을 신뢰성 있게 검증할 수 없어 컷오프를 강제하는 대신, 내용의 기준 시각은 항상 `asOf`가 진실이고 소비자는 `date`가 아니라 `asOf`·`factDate`로 신선도를 판단한다.
