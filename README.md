...
| 배치(`worker-batch`) | 종목마스터 적재·투자의견 수집→실시간 발행 동작. 수급·재무는 명세만 |
| 메인서버(`core-api`) | 인증·검색·관심목록·스트림·알림·커뮤니티·종목정보 REST 33개 동작. 남은 건 배포 경화(CORS·메트릭) |

다음 순서는 배치 잔여 잡(수급·재무), `core-api` 배포 경화, 부하 스모크다.
