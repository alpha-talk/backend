...
class MasterChangelogTest {
    ...
    fun `빈 PostgreSQL에 마스터 changelog 전체가 적용된다`() {
        withFreshDatabase("full_apply") { connection ->
            ...
            assertEquals(29, appliedChangeSetCount(connection))
            ...
            assertTrue(tableExists(connection, "financial_backfill"))
            assertTrue(columnExists(connection, "financial_backfill", "mapper_version"))
            ...
            assertTrue(tableExists(connection, "invest_opinion"))
            assertTrue(columnExists(connection, "stream_event", "source_key"))
            assertTrue(tableExists(connection, "opinion_read_cursor"))
        }
    }
