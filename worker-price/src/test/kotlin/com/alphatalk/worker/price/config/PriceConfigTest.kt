...
class PriceConfigTest {
    ...
    @Test
    fun `유효한 계정 JSON이면 계정 목록이 조립된다`() {
        val accounts = config.kisAccounts(PriceProperties(accountsJson = validAccounts))

        assertEquals(1, accounts.values.size)
        assertEquals("a1b2c3d4", accounts.values.first().keyId)
    }

    @Test
    fun `수요 소스는 게이트웨이 수요만 사용한다 - 초기 목표 집합은 공집합`() {
        val demandSource = RedisDemandSource(StringRedisTemplate(), LettuceConnectionFactory())
    ...
    fun `틱 구독 기본은 통합·시간외이고 KRX 전용은 종목별로 대체된다`() {
        assertEquals(listOf("H0UNCNT0", "H0STOUP0"), SessionPool.DEFAULT_TICK_TR_IDS)
    }
